...
# Maintains backwards compatibility while enabling capability-based configuration
{
  ...
  makeCapabilityHostConfig = platform: hostName: _system: _specialArgs: let
    ...
    finalModules =
      if hasCapabilities && capabilityConfig != null
      then
        # Use capability-based configuration
        capabilityConfig.imports
        ++ hostFiles
        ++ [
        ...
        if builtins.isList traditionalModules
        then traditionalModules
        else [traditionalModules];
  in {
    modules = finalModules;
    usingCapabilities = hasCapabilities;
    debug =
      if hasCapabilities && capabilityConfig != null
      then capabilityConfig._module.args.capabilityDebug or null
      else null;
  };
in {
  # Export the function
