...
let
  ...
  generateNodes = hostMappings:
    let
      # Host nodes
      hostNodes = lib.mapAttrsToList (hostName: hostData: {
        id = hostName;
        label = hostName;
        type = "host";
      ...
      capabilityNodes = lib.flatten (lib.mapAttrsToList (hostName: hostData:
        let
          capabilities = hostData.capabilities;
          features =
            lib.filterAttrs (_name: value: value == true) capabilities.features;
        in lib.mapAttrsToList (capName: _value: {
      ...
      categoryNodes = lib.flatten (lib.mapAttrsToList (hostName: hostData:
        map (category: {
          ...
          type = "category";
          host = hostName;
        }) hostData.categories) hostMappings);
    ...
    in hostNodes ++ capabilityNodes ++ categoryNodes ++ packageNodes;

  # Generate edges from host mapping data
  generateEdges = hostMappings:
    let
      # Host → Capability edges
      hostToCapEdges = lib.flatten (lib.mapAttrsToList (hostName: hostData:
        let
          capabilities = hostData.capabilities;
          features =
            lib.filterAttrs (_name: value: value == true) capabilities.features;
      ...
      catToPkgEdges = lib.flatten (lib.mapAttrsToList (hostName: hostData:
        lib.flatten (map (category:
          ...
          }) (hostData.packageNames or [ ])) hostData.categories))
        hostMappings);

    in hostToCapEdges ++ hostToCatEdges ++ catToPkgEdges;
...
in {
      ...
      nodeDOT = node:
        let
          ...
          style = {
            ...
            capability =
              "shape=ellipse, style=filled, fillcolor=lightcoral, fontsize=10";
            category =
              "shape=diamond, style=filled, fillcolor=orange, fontsize=12";
            package =
              "shape=circle, style=filled, fillcolor=lightgreen, fontsize=8";
          }.${node.type};
  ...
  getGraphStats = hostMappings:
    let
      nodes = generateNodes hostMappings;
      edges = generateEdges hostMappings;

      nodeTypeStats = lib.foldl'
        (acc: node: acc // { ${node.type} = (acc.${node.type} or 0) + 1; }) { }
        nodes;

      edgeTypeStats = lib.foldl'
        (acc: edge: acc // { ${edge.type} = (acc.${edge.type} or 0) + 1; }) { }
        edges;
    ...
    in {
      ...
      density =
        let maxPossibleEdges = (lib.length nodes) * (lib.length nodes - 1);
        in if maxPossibleEdges > 0 then
          (lib.length edges * 100) / maxPossibleEdges
        else
          0;
    };
}
