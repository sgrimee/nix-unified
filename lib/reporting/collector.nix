...
let
  ...
  collectHostCapabilities =
    ...
    let
      ...
      configCapabilities = if hostConfig ? hostCapabilities then
        hostConfig.hostCapabilities
      else if hostConfig ? capabilities then
        hostConfig.capabilities
      else if hostConfig ? config && hostConfig.config ? hostCapabilities then
        hostConfig.config.hostCapabilities
      else
        { };
    ...
    in {
      ...
      hardware = (capabilities.hardware or { }) // {
  ...
  collectHostPackageData = hostConfig: hostCapabilities: packageManager:
    let
      ...
      validation = if derivation != null then
        try (packageManager.validatePackages derivation.categories)
      else
        null;

      # Get package metadata if available
      metadata = if derivation != null then
        try (packageManager.getPackageInfo derivation.categories)
      else
        null;
    ...
    in {
      categories = if derivation != null then derivation.categories else [ ];
      ...
      metadata = if metadata != null then
        metadata
      else {
  ...
  collectHostMapping =
    ...
    in lib.mapAttrs extractHostDataSafe hostConfigs;

  # Generate system-wide overview from host mappings
  collectSystemOverview = hostMappings:
    let
      ...
      # Platform breakdown
      platformCounts = lib.foldl' (acc: platform:
        let
          count = lib.length
            (lib.filter (host: hostMappings.${host}.platform == platform)
              allHosts);
        in acc // { ${platform} = count; }) { } platforms;

      # Category usage statistics  
      categoryUsage = lib.foldl' (acc: category:
        let
          usage = lib.length (lib.filter
            (host: lib.elem category hostMappings.${host}.categories) allHosts);
        in acc // { ${category} = usage; }) { } allCategories;
...
in {
  ...
  inherit collectHostCapabilities collectHostPackageData collectHostMapping
