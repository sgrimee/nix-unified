  ...
  tests = {
    testGeneratePackages = {
      expr = let
        ...
        categories = if pkgs.stdenv.isDarwin then [
          "development"
          "productivity"
        ] else [
          "development"
    ...
    testValidatePackagesValid = {
      expr = let
        ...
        validCategories = if pkgs.stdenv.isDarwin then [
          "development"
          "productivity"
        ] else [
          "development"
    ...
    testGetPackageInfo = {
      ...
      in lib.elem "development" (map (c: c.name) info.categories);
      expected = true;
    };
  };
in tests
