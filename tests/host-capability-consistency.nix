...
# Ensures no drift between what hosts declare and what they actually do
{
  ...
  analyzeHostConfig = platform: hostName: let
    ...
    extractHints = filePath:
      ...
      in {
        hasGaming =
          lib.hasInfix "steam" content
          || lib.hasInfix "gaming" content;
        hasDesktop =
          lib.hasInfix "xserver" content
          || lib.hasInfix "sway" content
          || lib.hasInfix "gnome" content;
        hasDevelopment =
          lib.hasInfix "git" content
          || lib.hasInfix "vscode" content
          || lib.hasInfix "docker" content;
        hasMultimedia =
          lib.hasInfix "vlc" content
          || lib.hasInfix "ffmpeg" content;
        hasServer =
          lib.hasInfix "nginx" content
          || lib.hasInfix "apache" content
          || lib.hasInfix "postgres" content;
        hasHomeAssistant =
          lib.hasInfix "home-assistant" content
    ...
    combinedHints = {
      hasGaming =
        systemHints.hasGaming
        || packagesHints.hasGaming
        || homeHints.hasGaming;
      hasDesktop =
        systemHints.hasDesktop
        || packagesHints.hasDesktop
        || homeHints.hasDesktop;
      hasDevelopment =
        systemHints.hasDevelopment
        || packagesHints.hasDevelopment
        || homeHints.hasDevelopment;
      hasMultimedia =
        systemHints.hasMultimedia
        || packagesHints.hasMultimedia
        || homeHints.hasMultimedia;
      hasServer =
        systemHints.hasServer
        || packagesHints.hasServer
        || homeHints.hasServer;
      hasHomeAssistant =
        systemHints.hasHomeAssistant
        || packagesHints.hasHomeAssistant
        || homeHints.hasHomeAssistant;
      hasNvidia =
        systemHints.hasNvidia
        || packagesHints.hasNvidia
        || homeHints.hasNvidia;
      hasAMD = systemHints.hasAMD || packagesHints.hasAMD || homeHints.hasAMD;
      hasIntel =
        systemHints.hasIntel
        || packagesHints.hasIntel
        || homeHints.hasIntel;
      hasPipewire =
        systemHints.hasPipewire
        || packagesHints.hasPipewire
        || homeHints.hasPipewire;
      hasPulseaudio =
        systemHints.hasPulseaudio
        || packagesHints.hasPulseaudio
        || homeHints.hasPulseaudio;
      hasSSH = systemHints.hasSSH || packagesHints.hasSSH || homeHints.hasSSH;
      hasFirewall =
        systemHints.hasFirewall
        || packagesHints.hasFirewall
  ...
  compareCapabilityWithConfig = platform: hostName: let
    ...
    capabilityResult =
      if capabilityExists
      then builtins.tryEval (import capabilityPath)
      else {
        success = false;
        value = {};
      };
    ...
    consistencyChecks =
      ...
      then {
        gamingConsistent =
          (capabilities.features.gaming or false)
          == analysis.configurationAnalysis.hasGaming;
        desktopConsistent =
          (capabilities.features.desktop or false)
          == analysis.configurationAnalysis.hasDesktop;
        developmentConsistent =
          (capabilities.features.development or false)
          == analysis.configurationAnalysis.hasDevelopment;
        multimediaConsistent =
          (capabilities.features.multimedia or false)
          == analysis.configurationAnalysis.hasMultimedia;
        serverConsistent =
          (capabilities.features.server or false)
          == analysis.configurationAnalysis.hasServer;
        homeAssistantConsistent =
          (capabilities.services.homeAssistant or false)
        ...
        in
          if declaredGpu == "nvidia"
          then actualHasNvidia
          else if declaredGpu == "amd"
          then actualHasAMD
          else if declaredGpu == "intel"
          then actualHasIntel
          else true; # null or other values are ok
        ...
        in
          if declaredAudio == "pipewire"
          then actualHasPipewire
          else if declaredAudio == "pulseaudio"
          then actualHasPulseaudio
          else true;
        ...
        in
          declaredFirewall == actualHasFirewall;
      }
      else {};

    overallConsistent =
      if capabilityResult.success
      then lib.all (x: x) (lib.attrValues consistencyChecks)
      else false;
  in {
    hostName = hostName;
    platform = platform;
    ...
    inconsistencies =
      if capabilityResult.success
      then lib.filterAttrs (_name: value: !value) consistencyChecks
      else {};
  };
  ...
  generateConsistencyTests = platform: hosts:
    lib.listToAttrs (map (hostName: {
        ...
        value = {
          expr = let
            result = compareCapabilityWithConfig platform hostName;
          in
...
in
  lib.listToAttrs allConsistencyTests // generateSummaryTests
