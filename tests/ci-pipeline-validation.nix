...
# Ensures CI pipeline matches current host setup and will work correctly
{
  ...
  ciContent =
    if ciWorkflowExists
    then builtins.readFile ciWorkflowPath
    else "";
  ...
  analyzeCIWorkflow = content: let
    ...
    hasHostDiscovery =
      lib.hasInfix "Discover NixOS Hosts" content
      && lib.hasInfix "Discover Darwin Hosts" content;

    usesDynamicMatrix =
      lib.hasInfix "strategy:" content
    ...
    hasJustIntegration =
      lib.hasInfix "nixpkgs#just" content
      && lib.hasInfix "just " content;

    runsOnCorrectRunners =
      lib.hasInfix "runs-on: ubuntu-latest" content
      && lib.hasInfix "runs-on: macos-" content;

    hasProperTriggers =
      lib.hasInfix "on:" content
      && lib.hasInfix "push:" content
      && lib.hasInfix "pull_request:" content;
  in {
    inherit
      hasTestJob
      hasLintJob
  ...
  ciAnalysis =
    if ciWorkflowExists
    then analyzeCIWorkflow ciContent
    else {
      hasTestJob = false;
      hasLintJob = false;
  ...
  validateHostDiscovery = let
    ...
    usesHostMatrix =
      lib.hasInfix "fromJson" ciContent
  ...
  checkCIIssues = content: let
    # Security issues
    hasSecretsExposed =
      lib.hasInfix "\${{" content
    ...
    # Dependency issues
    hasVersionPinning =
      lib.hasInfix "@v" content
      || lib.hasInfix "@main" content;

    # Error handling
    hasErrorHandling =
      lib.hasInfix "|| true" content
...
in {
  ...
  testCIWorkflowNotEmpty = {
    expr =
      if ciWorkflowExists
      then (builtins.stringLength ciContent) > 500
      else false;
    expected = true;
  };
  ...
  testDiscoveryMatchesActualHosts = {
    expr =
      hostDiscoveryValidation.totalHostCount
      > 0
