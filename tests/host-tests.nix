{
  ...
  generateHostTests = platform: hostName: let
    requiredFiles = ["system.nix" "home.nix" "capabilities.nix"];
    tests =
      map (fileName: generateHostFileTest platform hostName fileName)
      requiredFiles;
  in
  ...
  allHostTests = lib.flatten [
    (lib.mapAttrsToList (platform: hosts:
      lib.flatten (map (hostName: let
        hostTests = generateHostTests platform hostName;
      in
...
in
  ...
  // {
    ...
    testNixOSHostsExist = {
      expr =
        builtins.hasAttr "nixos" allHosts
        && builtins.length allHosts.nixos
        > 0;
      expected = true;
    };
    ...
    testDarwinHostsExist = {
      expr =
        builtins.hasAttr "darwin" allHosts
        && builtins.length allHosts.darwin
        > 0;
      expected = true;
    };
    ...
    testKnownHosts = {
      ...
      in
        hasNixair && hasDracula && hasLegion && hasCirice && hasSGRIMEE;
      expected = true;
    };
    ...
    testAllHostsHaveCapabilities = {
      ...
      in
        nixosCompliant && darwinCompliant;
      expected = true;
    };
    ...
    testNoModuleDefaultFiles = {
      ...
      in
        darwinDefaultGone && nixosDefaultGone && homeManagerDefaultExists;
      expected = true;
    };
    ...
    testCapabilityFilesValid = {
      expr = let
        checkCapabilityFile = platform: hostName: let
          capFile = ../hosts/${platform}/${hostName}/capabilities.nix;
        in
          if builtins.pathExists capFile
          then builtins.tryEval (import capFile)
          else {
            success = false;
          };
