{
  ...
  getModulesForPlatform = platform: let
    modulesDir = ../../modules + "/${platform}";
  in
  ...
  testModuleCompatibility = module1: module2: platform: let
    ...
    shouldSkipTest =
      ...
      || module1 == "default"
      || module2 == "default"
      || lib.hasInfix ".nix" module1
      || lib.hasInfix ".nix" module2;
  ...
  in {
    ...
    error =
      ...
      then null
      else "Module conflict detected";
  };

  # Test that a module works on a specific platform
  testModuleOnPlatform = module: platform: let
  ...
  testPackageConflicts = packages: let
    uniquePackages = lib.unique packages;
    hasConflicts =
      (builtins.length uniquePackages)
  ...
  testDependencyResolution = module: platform: requiredDeps: let
    # Skip problematic modules
    shouldSkipTest =
      module
      == "default.nix"
  ...
  generateModuleCombinationTests = platform: let
  ...
  testCrossPlatformPortability = moduleName: let
  ...
  verifySystemServices = services: ''
    ...
    ${lib.concatMapStringsSep "\n"
      (service: ''machine.succeed("systemctl is-active ${service}")'')
      services}
  '';
...
in {
  inherit
    ...
    testModuleOnPlatform
    testPackageConflicts
    testDependencyResolution
    measureBuildTime
    generateModuleCombinationTests
    testCrossPlatformPortability
    runIntegrationTest
    verifySystemServices
    verifyHomeManagerConfig
    allDarwinModules
    allNixOSModules
