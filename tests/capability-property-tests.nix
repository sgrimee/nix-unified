...
# Validates that capability combinations are logically consistent
{
  ...
  testCapabilityCombination = testCase: let
    ...
    platformConsistent = let
      ...
      darwinValid =
        if isDarwin
        then
          testCase.hardware.audio
          == "coreaudio"
          && (testCase.environment.desktop
            == "macos"
            || testCase.environment.desktop == null)
          && (!testCase.features.gaming
            || testCase.features.gaming
    ...
    featureLogicConsistent = let
      ...
      desktopConsistent =
        if testCase.environment.desktop != null
        then features.desktop
        else true;
    in
      gamingImpliesMultimedia && serverGamingConflict && desktopConsistent;
    ...
    hardwareConsistent = let
      ...
      appleHardwareOnDarwin =
        if hardware.cpu == "apple"
        then testCase.platform == "darwin"
        else true;

      # Servers might not need GPU
      gpuConsistent =
        if testCase.features.server && !testCase.features.desktop
        then hardware.gpu == null || hardware.gpu != null
        else true;

      # Gaming rigs should have dedicated GPU
      gamingHasGPU =
        if testCase.features.gaming
        then hardware.gpu != null && hardware.gpu != "intel"
        else true;
    in
      appleHardwareOnDarwin && gpuConsistent && gamingHasGPU;
    ...
    roleConsistent = let
      ...
      workstationHasFeatures =
        if lib.elem "workstation" roles
        then testCase.features.development || testCase.features.desktop
        else true;
    in
      noConflictingRoles && workstationHasFeatures;
    ...
    serviceConsistent = let
      ...
      homeAssistantOnServer =
        if services.homeAssistant
        then testCase.features.server || lib.elem "home-server" testCase.roles
        else true;

      # Docker implies development or server features
      dockerImpliesFeatures =
        if services.development.docker
        then testCase.features.development || testCase.features.server
        else true;
    in
      homeAssistantOnServer && dockerImpliesFeatures;
  ...
  generateCombinationTests =
    map (result: {
      name = "test${lib.strings.toUpper (builtins.substring 0 1 result.testCase)}${
  ...
  in {
    ...
    commonFeatures = let
      ...
      featureCounts =
        ...
        uniqueFeatures;
    in
      lib.sort (a: b: a.count > b.count) featureCounts;
...
in
  lib.listToAttrs generateCombinationTests
  // {
    ...
    testPlatformCoverage = {
      expr =
        propertyAnalysis.platformDistribution.nixos
        > 0
    ...
    testGamingMultimediaRule = {
      expr = let
        gamingCombinations =
          ...
          allTestResults;
        allHaveMultimedia =
          lib.all (r: lib.elem "multimedia" r.details.features)
          gamingCombinations;
      in
        (lib.length gamingCombinations) == 0 || allHaveMultimedia;
      expected = true;
    };
    ...
    testRoleFeatureConsistency = {
      expr = let
        ...
        workstationsHaveRelevantFeatures = lib.all (r:
          ...
          || lib.elem "desktop" r.details.features)
        workstationRoles;
      in
        (lib.length workstationRoles) == 0 || workstationsHaveRelevantFeatures;
      expected = true;
    };
    ...
    testConflictingCombinationRejected = {
      expr = let
        ...
        conflictDetected =
          ...
          then !conflictingTest.overallValid
          else false;
      in
        conflictDetected;
      expected = true;
