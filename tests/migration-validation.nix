...
# Ensures no functionality is lost during capability system migration
{
  ...
  buildWithCapabilities = hostName: platform: let
  ...
  in {
    identical =
      lib.sort (a: b: a < b) preServiceNames
  ...
  compareConfigurations = pre: post: let
    # Extract key configuration sections for comparison
    extractKeyConfig = config: {
      services =
        lib.mapAttrs (_name: svc: svc.enable or false)
        (config.services or {});
      programs =
        lib.mapAttrs (_name: prog: prog.enable or false)
        (config.programs or {});
      hardware = {
...
in rec {
    ...
    nixos = lib.mapAttrs (hostName: preHost:
      ...
      in
        ...
        then let
          ...
          # Service comparison
          serviceComparison =
            compareServices (preHost.enabledServices or {})
          ...
          featureParity = {
            ...
            configuration =
              configComparison.servicesMatch
              && configComparison.programsMatch
              && configComparison.hardwareMatch;
          };

          overallParity =
            featureParity.packages
            && featureParity.services
    ...
    darwin = lib.mapAttrs (hostName: preHost:
      ...
      in
        ...
        then let
          ...
          overallParity =
            featureParity.systemPackages
            && featureParity.homebrewBrews
            && featureParity.homebrewCasks;
        in {
          hostName = hostName;
          platform = "darwin";
  ...
  migrationSummary = let
    allParityResults =
      (lib.attrValues testFeatureParity.nixos)
      ++ (lib.attrValues testFeatureParity.darwin);
    allBuildResults =
      (lib.attrValues testBuildSuccess.nixos)
  ...
  in {
    ...
    migrationSuccessRate =
      if (lib.length allParityResults) > 0
      then (lib.length successfulParity) / (lib.length allParityResults)
      else 0;

    buildSuccessRate =
      if (lib.length allBuildResults) > 0
      then (lib.length successfulBuilds) / (lib.length allBuildResults)
      else 0;
  };
  ...
  failureAnalysis = {
    ...
    buildFailures =
      lib.filterAttrs (_hostName: result: !result.success)
