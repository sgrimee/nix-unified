...
let
  ...
  testHostEvaluation = platform: hostName:
    let
      ...
      hostEvalResult = if hostExists then
        builtins.tryEval (import hostPath { inputs = { }; })
      else {
        success = false;
      ...
      systemEvalResult = if systemExists then
    ...
    in {
      ...
      hostEvalError =
        if !hostEvalResult.success then hostEvalResult.value else null;
      systemEvalError =
        if !systemEvalResult.success then systemEvalResult.value else null;
  ...
  testFlakeOutputs = let
    ...
    flakeEvalResult = if flakeExists then
      builtins.tryEval (import flakePath)
    else {
      success = false;
  ...
  in {
    ...
    evalError =
      if !flakeEvalResult.success then flakeEvalResult.value else null;
  };
  ...
  testModuleIntegration = let
    ...
    darwinDefaultEval = if darwinDefaultExists then
      builtins.tryEval
    ...
    nixosDefaultEval = if nixosDefaultExists then
      builtins.tryEval
    ...
    homeManagerDefaultEval = if homeManagerDefaultExists then
      builtins.tryEval
  ...
  in {
    directoriesExist = darwinModulesExist && nixosModulesExist
      && homeManagerModulesExist;
    defaultFilesExist = darwinDefaultExists && nixosDefaultExists
      && homeManagerDefaultExists;
    ...
    allModulesEvaluate = darwinDefaultEval.success && nixosDefaultEval.success
      && homeManagerDefaultEval.success;

    errors = lib.filter (e: e != null) [
      (if !darwinDefaultEval.success then
        "Darwin: ${darwinDefaultEval.value}"
      else
        null)
      (if !nixosDefaultEval.success then
        "NixOS: ${nixosDefaultEval.value}"
      else
        null)
      (if !homeManagerDefaultEval.success then
        "Home Manager: ${homeManagerDefaultEval.value}"
      else
        null)
    ];
  };
  ...
  testPackageManagement = let
    ...
    managerEval = if managerExists then
    ...
    discoveryEval = if discoveryExists then
      builtins.tryEval (import discoveryPath { inherit lib; })
    else {
      success = false;
  ...
  testCapabilitySystem = let
    ...
    schemaEval = if lib.elem true (lib.take 1 filesExist) then
      builtins.tryEval
    ...
    loaderEval =
      if lib.length filesExist > 1 && lib.elem true (lib.take 2 filesExist) then
        builtins.tryEval
  ...
  testUtilityScripts = let
    ...
    scriptsExist = map (script: builtins.pathExists (utilsDir + "/${script}"))
      expectedScripts;
  ...
  in {
    ...
    missingScripts = lib.subtractLists
  ...
  generateHostTests = platform: hosts:
    lib.listToAttrs (map (hostName:
      let result = testHostEvaluation platform hostName;
      in {
  ...
  allHostIntegrationTests = lib.flatten [
    (lib.mapAttrsToList (platform: hosts:
      let hostTests = generateHostTests platform hosts;
  ...
  hostTestResults = lib.flatten [
    (lib.mapAttrsToList (platform: hosts:
      map (hostName: testHostEvaluation platform hostName) hosts) allHosts)
  ];
...
in lib.listToAttrs allHostIntegrationTests // {
  ...
  testModuleSystemIntegration = {
    expr = moduleTestResult.directoriesExist
  ...
  testPackageSystemIntegration = {
    expr = packageTestResult.packagesSystemExists
  ...
  testUtilitySystemIntegration = {
    expr = utilityTestResult.utilsDirectoryExists
  ...
  testOverallSystemHealth = {
    expr = let
      ...
      subsystemsHealthy = flakeTestResult.flakeEvaluates
        && moduleTestResult.allModulesEvaluate
        && packageTestResult.packageSystemWorking
        && capabilityTestResult.capabilitySystemWorking
        && utilityTestResult.allScriptsPresent;
    in hostsHealthy && subsystemsHealthy;
    expected = true;
  };
  ...
  testAllHostsEvaluate = {
    ...
    in totalHosts > 0 && successfulHosts == totalHosts;
    expected = true;
  };
  ...
  testMinimumSystemRequirements = {
    ...
    in hasHosts && hasFlake && hasModules && hasPackages && hasCapabilities
    && hasUtils;
    expected = true;
  };
  ...
  testCrossPlatformCompatibility = {
    expr = let
      ...
      nixosWorking = (lib.length nixosHosts) > 0
        && lib.all (r: r.overallSuccess) nixosHosts;
      darwinWorking = (lib.length darwinHosts) > 0
        && lib.all (r: r.overallSuccess) darwinHosts;
    in nixosWorking && darwinWorking;
    expected = true;
  };
  ...
  testSystemIntegration = {
    expr = let
      ...
      systemsIntegrated = flakeTestResult.flakeEvaluates
        && moduleTestResult.allModulesEvaluate;
      hostsIntegrated =
        lib.all (r: r.hostExists && r.systemExists) hostTestResults;
    in systemsIntegrated && hostsIntegrated;
    expected = true;
  };
}
