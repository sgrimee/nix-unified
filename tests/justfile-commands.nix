...
let
  ...
  justfileContent =
    if justfileExists then builtins.readFile justfilePath else "";
  ...
  extractRecipes = content:
    let
      ...
      recipeLines = lib.filter (line:
        let trimmed = lib.strings.removePrefix " " line;
        in trimmed == line && lib.hasInfix ":" line && !lib.hasPrefix "#" line
        && !lib.hasPrefix "default" line) lines;

      # Extract recipe names (everything before the colon)
      recipeNames = map (line:
        let parts = lib.splitString ":" line;
        in lib.strings.trim (lib.head parts)) recipeLines;
    in lib.unique recipeNames;
  ...
  extractCommandDependencies = command:
    let
      lines = lib.splitString "\n" justfileContent;

      # Find the command definition line
      commandLine = lib.findFirst (line:
        let trimmed = lib.strings.trim line;
        in lib.hasPrefix "${command}" trimmed && lib.hasInfix ":" trimmed) null
        lines;

      # Extract any parameters or dependencies mentioned in the command line
      hasDependencies = if commandLine != null then
        lib.hasInfix "HOST" commandLine || lib.hasInfix "ARGS" commandLine
      else
        false;
  ...
  analyzeCommandCoverage = let
    ...
    maintenanceCommands = lib.filter (cmd:
      lib.hasInfix "lint" cmd || lib.hasInfix "fmt" cmd || lib.hasInfix "gc" cmd
      || lib.hasInfix "update" cmd) discoveredRecipes;
  ...
  generateCommandTests = map (command: {
  ...
  generateSyntaxTests = map (command:
    let structure = testCommandStructure command;
    in {
      name = "test${lib.strings.toUpper (builtins.substring 0 1 command)}${
          builtins.substring 1 (builtins.stringLength command) command
        }Syntax";
...
in lib.listToAttrs (generateCommandTests ++ generateSyntaxTests) // {
  ...
  testJustfileNotEmpty = {
    expr = if justfileExists then
      (builtins.stringLength justfileContent) > 100
    else
      false;
    expected = true;
  };
  ...
  testCommandNamingConsistency = {
    ...
    in testConventionValid && (lib.length testCommands) > 0;
    expected = true;
  };
