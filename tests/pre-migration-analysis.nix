...
# Ensures we can validate feature parity after migration
{
  ...
  extractModuleImports = config: let
    getImports = cfg:
      if cfg ? imports && builtins.isList cfg.imports
      then cfg.imports ++ (lib.flatten (map getImports cfg.imports))
      else [];
  in
    lib.unique (getImports config);

  # Extract system packages from a built configuration
  extractSystemPackages = hostConfig: let
    evalResult = safeEval hostConfig.config.environment.systemPackages;
  in
  ...
  extractEnabledServices = hostConfig: let
    services = hostConfig.config.services;
    enabledServices = lib.filterAttrs (_name: service:
      if builtins.isAttrs service && service ? enable
      then service.enable
      else false)
    services;
  in
    lib.mapAttrs (_name: service: {
  ...
  extractHardwareConfig = hostConfig: let
    hardware = hostConfig.config.hardware or {};
  in {
  ...
  extractUserConfig = hostConfig: let
    users = hostConfig.config.users or {};
  in {
...
in rec {
  ...
  baseline = {
    ...
    in
      ...
      then {
        ...
        moduleImports =
          if hostExists
          then extractModuleImports (import hostPath)
          else [];
        ...
        desktopEnvironment =
          if config.config.services.xserver.desktopManager.gnome.enable or false
          then "gnome"
          else if config.config.programs.sway.enable or false
          then "sway"
          else if config.config.services.xserver.desktopManager.kde.enable or false
          then "kde"
          else null;

        # Display server detection
        displayServer =
          if config.config.services.xserver.enable or false
          then "x11"
          else if config.config.programs.sway.enable or false
          then "wayland"
          else null;
        ...
        gaming = {
          ...
          gamemode =
            lib.any (pkg: pkg.pname or "" == "gamemode")
            (extractSystemPackages config);
        };
      }
      else {
    ...
    # Darwin hosts
    darwin = lib.mapAttrs (hostName: hostConfig: let
      config = hostConfig;
      evalResult = safeEval config;
    in
      if evalResult.success
  ...
  analysis = {
    ...
    getAllModuleImports = let
      ...
      darwinImports =
        ...
        (lib.mapAttrsToList (_name: host: host.moduleImports or [])
          baseline.darwin);
    in
      lib.unique (nixosImports ++ darwinImports);
    ...
    identifyCapabilityPatterns = {
      ...
      desktopEnvironments = lib.unique (lib.flatten [
        (lib.mapAttrsToList (_name: host:
          if host.desktopEnvironment != null
          then [host.desktopEnvironment]
          else [])
        baseline.nixos)
      ]);
  ...
  validation = {
    ...
    in {
      ...
      total =
        (lib.length (lib.attrNames nixosFailures))
    ...
    statistics = {
      totalHosts =
        (lib.length (lib.attrNames baseline.nixos))
      ...
      successfulBuilds =
        lib.length (lib.filter (host: host.buildSuccess)
