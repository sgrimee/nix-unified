# packages/manager.nix
{
  ...
  currentGpu =
    if (hostCapabilities.hardware.gpu or null) == null
    then "integrated"
    else hostCapabilities.hardware.gpu;
in {
  ...
  generatePackages = requestedCategories: let
    ...
    requestedCategories;
  in
    lib.unique (lib.flatten categoryPackages);

  # Generate package names (for analysis/reporting)
  generatePackageNames = requestedCategories: let
    ...
    categoryPackages = map (category:
      ...
      in
        map (pkg:
          ...
          then pkg.pname or pkg.name or "unknown-package"
          else toString pkg)
        packages
      else [])
    requestedCategories;
  ...
  validatePackages = requestedCategories: let
    # Check for conflicts
  ...
  getPackageInfo = requestedCategories: let
    totalSize =
