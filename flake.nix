{
  ...
  in {
    ...
    hostPackageMapping = let
      ...
      loadCapabilities = platform: hostName: let
        capPath = ./hosts + "/${platform}/${hostName}/capabilities.nix";
      in
        if builtins.pathExists capPath
        then import capPath
        else {};

      capabilityData =
        (lib.genAttrs (lib.attrNames nixosHosts)
      ...
      packageManagerFactory = hostCapabilities: let
        overlays = import ./overlays;
        # Use stable packages with overlays for package derivation
        pkgs =
          if hostCapabilities.platform or "" == "darwin"
          then
            import inputs.stable-darwin {
    ...
    in {
      ...
      exportGraphML =
        reporting.exportGraphML
        (reporting.collectAllHosts allHostConfigs platformMapping
          capabilityData
          packageManagerFactory).hosts;
      exportDOT =
        reporting.exportDOT
        (reporting.collectAllHosts allHostConfigs platformMapping
          capabilityData
