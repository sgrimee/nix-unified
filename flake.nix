{
  ...
  in {
    ...
    # Custom library outputs for package discovery and management
    lib = {
      packageCategories =
    ...
    checks = {
      ...
      in {
        ...
        capability-system-tests =
          pkgs.runCommand "capability-system-tests"
          {
            ...
            if [ -d "$src/hosts/nixos" ]; then
              for host_dir in $src/hosts/nixos/*/; do
                if [ -d "$host_dir" ]; then
                  host=$(basename "$host_dir")
                  if [ -f "$src/hosts/nixos/$host/capabilities.nix" ]; then
            ...
            if [ -d "$src/hosts/darwin" ]; then
              for host_dir in $src/hosts/darwin/*/; do
                if [ -d "$host_dir" ]; then
                  host=$(basename "$host_dir")
                  if [ -f "$src/hosts/darwin/$host/capabilities.nix" ]; then
                    echo "✓ $host capabilities exist" >> $out
                  else
                    echo "✗ $host capabilities missing" >> $out
                  fi
            ...
            if [ -f "$src/lib/capability-schema.nix" ]; then
            ...
            if [ -f "$src/lib/capability-system.nix" ]; then
            ...
            if [ -d "$src/lib/module-mapping" ] && [ -f "$src/lib/module-mapping/default.nix" ]; then
        ...
        pre-migration-tests =
          pkgs.runCommand "pre-migration-tests"
          {
            src = ./.;
          } ''
            ...
            if [ -f "$src/tests/pre-migration-analysis.nix" ]; then
            ...
            if [ -f "$src/tests/migration-validation.nix" ]; then
            ...
            if [ -f "$src/tests/capability-tests.nix" ]; then
        ...
        test-utils =
          pkgs.runCommand "test-utils-validation"
          {
            src = ./.;
          } ''
            ...
            if [ -f "$src/tests/lib/test-utils.nix" ]; then
              echo "✓ test-utils.nix file exists" >> $out
              # Count the number of functions exported
              functions=$(grep -c "^\s*[a-zA-Z_][a-zA-Z0-9_]*\s*=" "$src/tests/lib/test-utils.nix" || echo "0")
        ...
        module-discovery =
          pkgs.runCommand "module-discovery"
          {
            src = ./.;
          } ''
            echo "=== Module Discovery Test ===" > $out
            echo "Darwin modules: $(find "$src/modules/darwin" -name "*.nix" | wc -l)" >> $out
            echo "NixOS modules: $(find "$src/modules/nixos" -name "*.nix" | wc -l)" >> $out
            echo "Home Manager modules: $(find "$src/modules/home-manager" -name "*.nix" | wc -l)" >> $out
            echo "All modules discoverable: ✓" >> $out
          '';

        # Test that hosts can be built
        host-build-test =
          pkgs.runCommand "host-build-test"
          {
            src = ./.;
          } ''
            ...
            if [ -d "$src/hosts/nixos" ]; then
              for host in $src/hosts/nixos/*/; do
                hostname=$(basename "$host")
                echo "NixOS host: $hostname" >> $out
              done
            fi
            if [ -d "$src/hosts/darwin" ]; then
              for host in $src/hosts/darwin/*/; do
                hostname=$(basename "$host")
        ...
        core-test-structure =
          pkgs.runCommand "core-test-structure"
          {
            src = ./.;
          } ''
            echo "=== Core Test Structure ===" > $out
            echo "Core Test Files: $(find "$src/tests" -name "*.nix" 2>/dev/null | wc -l)" >> $out
            echo "Basic tests: $([ -f "$src/tests/basic.nix" ] && echo "✓" || echo "✗")" >> $out
            echo "Config validation: $([ -f "$src/tests/config-validation.nix" ] && echo "✓" || echo "✗")" >> $out
            echo "Module tests: $([ -f "$src/tests/module-tests.nix" ] && echo "✓" || echo "✗")" >> $out
            echo "Host tests: $([ -f "$src/tests/host-tests.nix" ] && echo "✓" || echo "✗")" >> $out
            echo "Test structure validation: ✓" >> $out
          '';

        # Test CI matrix validation
        ci-matrix-test =
          pkgs.runCommand "ci-matrix-test"
          {
            src = ./.;
          } ''
            echo "=== CI Matrix Test ===" > $out
            echo "CI workflow exists: $([ -f "$src/.github/workflows/ci.yml" ] && echo "✓" || echo "✗")" >> $out
        ...
        justfile-test =
          pkgs.runCommand "justfile-test"
          {
            src = ./.;
          } ''
            echo "=== Justfile Test ===" > $out
            echo "Justfile exists: $([ -f "$src/justfile" ] && echo "✓" || echo "✗")" >> $out
      ...
      in {
        ...
        darwin-tests =
          pkgs.runCommand "darwin-tests"
          {
            src = ./.;
          } ''
            echo "=== Darwin Tests ===" > $out
            echo "Testing Darwin platform..." >> $out
            echo "Darwin modules available: $(find "$src/modules/darwin" -name "*.nix" | wc -l)" >> $out
        ...
        darwin-module-tests =
          pkgs.runCommand "darwin-module-tests"
          {
            ...
            find "$src/modules/darwin" -name "*.nix" -exec echo "✓ {}" \; >> $out
