...
let cfg = config.virtualization.windowsGpuPassthrough;
in {
  options.virtualization.windowsGpuPassthrough = {
    ...
    iommuType = lib.mkOption {
      ...
      default = "amd_iommu";
      description =
        "IOMMU type for the CPU (intel_iommu for Intel, amd_iommu for AMD)";
    };
    ...
    vfioIds = lib.mkOption {
      ...
      description =
        "PCI IDs of devices to bind to VFIO driver (GPU and audio controller)";
    };
    ...
    lookingGlass = {
      ...
      shmSize = lib.mkOption {
        ...
        example = "1G";
        description =
          "Shared memory size for Looking Glass (64M, 128M, 256M, 512M, 1G, 2G)";
      };
    };
  ...
  config = lib.mkIf cfg.enable {
    ...
    boot = {
      ...
      kernelParams = [ "${cfg.iommuType}=on" "iommu=pt" "kvm.ignore_msrs=1" ]
        ++ lib.optionals (cfg.vfioIds != [ ])
        [ ("vfio-pci.ids=" + lib.concatStringsSep "," cfg.vfioIds) ];

      # Ensure VFIO loads early
    ...
    systemd.tmpfiles.rules = lib.mkIf cfg.lookingGlass.enable
    ...
    boot.kernel.sysctl = lib.mkIf cfg.lookingGlass.enable (let
      ...
      shmBytes = if cfg.lookingGlass.shmSize == "2G" then
        2147483648
      else if cfg.lookingGlass.shmSize == "1G" then
        1073741824
      else if cfg.lookingGlass.shmSize == "512M" then
        536870912
      else if cfg.lookingGlass.shmSize == "256M" then
        268435456
      else if cfg.lookingGlass.shmSize == "128M" then
        134217728
      else if cfg.lookingGlass.shmSize == "64M" then
        67108864
      else
        134217728; # Default to 128MB
    in {
