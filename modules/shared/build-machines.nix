...
let
  ...
  registry = {
    cirice = {
      ...
      sshUser = "sgrimee";
      hostKey =
        "cirice.local ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILl1KxPJWXWG6vNM0D1RdFYNXlzRsmhEorXciUxpDz3x";
      system = "x86_64-linux";
      maxJobs = 8;
      speedFactor = 100;
    ...
    legion = {
      ...
      sshUser = "sgrimee";
      hostKey =
        "legion.local ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPrvPdkufPSh0AIZ/fIhtWnkwsjWlzjt9EjD+byl68gK";
      system = "x86_64-linux";
      maxJobs = 8;
      speedFactor = 50;
      supportedFeatures = [ "kvm" "nixos-test" "big-parallel" ];
  ...
  mkNixOSBuildMachines = enabledNames:
    let machines = getEnabledMachines enabledNames;
    in map (machine: {
  ...
  mkDarwinBuilders = enabledNames:
    let
      machines = getEnabledMachines enabledNames;
      machineStrings = map (machine:
        "ssh://${machine.sshUser}@${machine.hostName} ${machine.system} /Users/sgrimee/.ssh/id_ed25519 ${
          toString machine.maxJobs
        } ${toString machine.speedFactor} ${
          lib.concatStringsSep "," machine.supportedFeatures
        }") machines;
    in lib.concatStringsSep " ; " machineStrings;

  # Generate SSH known_hosts entries
  mkHostKeys = enabledNames:
    let machines = getEnabledMachines enabledNames;
